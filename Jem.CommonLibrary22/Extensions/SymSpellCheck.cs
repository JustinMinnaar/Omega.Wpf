...
public static class SymSpell
{
    ...
    private class SuggestItem
    {
        ...
        {
            return Equals(term, (obj as SuggestItem)?.term);
        }
    ...
    private static bool CreateDictionaryEntry(string key, string language)
    {
        ...
        if (dictionary.TryGetValue(language + key, out object? valueo))
        {
            //int or dictionaryItem? delete existed before word!
            if (valueo is Int32 tmp) { value = new DictionaryItem(); value.suggestions.Add(tmp); dictionary[language + key] = value; }
        ...
        {
            if (dictionary.TryGetValue(language + delete, out object? value2))
            {
                ...
                if (value2 is Int32 tmp)
                {
                    DictionaryItem di = new DictionaryItem(); di.suggestions.Add(tmp); dictionary[language + delete] = di;
    ...
    private static List<SuggestItem> Lookup(string input, string language, int editDistanceMax)
    {
        ...
        while (candidates.Count > 0)
        {
            ...
            if (dictionary.TryGetValue(language + candidate, out object? valueo))
            {
                ...
                foreach (int suggestionint in value.suggestions)
                {
                    ...
                    if (hashset2.Add(suggestion))
                    {
                        ...
                        {
                            if (dictionary.TryGetValue(language + suggestion, out object? value2))
                            {
                                SuggestItem si = new SuggestItem
                                {
                                    term = suggestion,
                                    count = ((DictionaryItem)value2).count,
                                    distance = distance
                                };
                                suggestions.Add(si);
                            }
    ...
    public static Int32 DamerauLevenshteinDistance(this String source, String target)
    {
        if (source == null) return (target == null ? 0 : target.Length);
